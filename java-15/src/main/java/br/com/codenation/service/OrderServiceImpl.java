...
public class OrderServiceImpl implements OrderService {
		...
		return items.stream().mapToDouble(orderItem ->
			{
				...
				if(product != null){
					return (orderItem.getQuantity() * product.getValue()) * (product.getIsSale() ? DISCOUNT : NO_DISCOUNT);
				}
				return 0;
			}).sum();
	}
	...
	public Map<Boolean, List<Product>> groupProductsBySale(List<Long> productIds) {
		return productIds.stream().map(productAgrupados -> productRepository.findById(productAgrupados).orElse(null))
				.filter(Objects::nonNull)
